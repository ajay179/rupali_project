...
$(".isAlpha").keypress(function (event) {  // only alphabets with space and percentage sign
     event = (event) ? event : window.event;
     var charCode = (event.which) ? event.which : event.keyCode;
     if (charCode != 8 && charCode != 9 && charCode != 46 && (charCode < 97 || charCode > 122) && (charCode < 65 || charCode > 90) && charCode != 39 && charCode != 32 || charCode == 37 || charCode == 38) {
          return false;
     }
     return true;
});
$(".isAlphaSpecialChar").keypress(function (event) {  // only alphabets with space and percentage sign
     event = (event) ? event : window.event;
     var charCode = (event.which) ? event.which : event.keyCode;
